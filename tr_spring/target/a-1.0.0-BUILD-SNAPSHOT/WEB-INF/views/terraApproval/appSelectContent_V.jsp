...
<%
	...
	ArrayList<AppVacationVO> vList = (ArrayList<AppVacationVO>)request.getAttribute("vList"); // 휴가신청서 
	...
	int uplevel = Integer.parseInt(vList.get(0).getAlevel()) + 1 ;
	String step = CodeUtil.nowStep(vList.get(0).getAlevel(), vList.get(0).getAapp(), uList.get(0).getMnum());

	
	AppVacationVO avvo = vList.get(0);
	String app[] = avvo.getAapp().split(",");
	...
	String[] confirm = vList.get(0).getAconfirm().split(",");
	String ref = "";
...
$(document).ready(function(){
	...
	if (<%=f%> == -1 || <%=Integer.parseInt(vList.get(0).getAlevel())%> == 9) {
	...
	$(document).on('click', '#approvalproxy', function() {
		...
		<%if(pList != null){
			step = CodeUtil.nowStep(vList.get(0).getAlevel(), vList.get(0).getAapp(), pList.get(0).getMnum());
		}%>
		if ('<%=step%>' == '대기') {
			var num = $('#alevel').val('<%=uplevel%>');
...
<table>
	<tr>
		...
		<td colspan="2">&nbsp;<%=avvo.getAnum() %></td>
	</tr>
	<tr>
		<td class="mpo">기안 부서</td>
		<td colspan="2">&nbsp;<%=CodeUtil.dappVal(avvo.getMdeptno())%></td>
		<td class="mpo">기안자</td>
		<td colspan="2">&nbsp;<%=avvo.getMname()%></td>
	</tr>
	<tr>
		<td class="mpo">보존 연한</td>
		<td colspan="2">&nbsp;<%=avvo.getDeletedate()%> 년</td>
		<td class="mpo">보안 등급</td>
		<td colspan="2">&nbsp;<%=CodeUtil.aauthVal(avvo.getAauth())%></td>
	</tr>
	<tr>
		<td class="mpo">기안 일시</td>
		<td colspan="2">&nbsp;<%=avvo.getInsertdate()%></td>
		<td class="mpo">완료 일시</td>
		<td colspan="2">&nbsp;<%=avvo.getFindate()%></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td style="text-align:center;">
			...
			<%}else if(app[i].equals(uList.get(0).getMnum())|| app[i].equals(pList.get(0).getMnum()) && !avvo.getAlevel().equals("00")){ %>
		...
		<td style="text-align:center;">
			...
			<%}else if (confirm[i].equals(app[i])) { %>
				<img style="width:50px;height:50px;" src="../resources/images/evaluate.png"/>
				<input type="hidden" class="confirms" value="<%=app[i]%>"/>
			<%}else if(app[i].equals(uList.get(0).getMnum()) && !avvo.getAlevel().equals("00")){ %>
	...
	<table>
		<tr>
			<td class="mpo" style="height:40px">휴가시작일</td>
			<td style="height:40px">&nbsp;<%= avvo.getStartdate() %></td>
			<td class="mpo" style="height:40px">휴가종료일</td>
			<td style="height:40px">&nbsp;<%= avvo.getEnddate() %></td>
			<td class="mpo" style="height:40px">잔여휴가</td>
			<td style="height:40px">&nbsp;<%= mvo.getMholiday() %>일</td>
		</tr>
	</table>
	<br>
	<div style="width:780px"><h2><%=avvo.getVsubject()%></h2>
	<hr>
	<p><%=avvo.getVmemo()%></p></div>
	<input type="hidden" id="alevel" name="alevel"/>
	<input type="hidden" id="aconfirm" name="aconfirm"/>
	<input type="hidden" id="aapp" name="aapp" value="<%=avvo.getAapp()%>"/>
	<input type="hidden" id="anum" name="anum" value="<%=avvo.getAnum()%>"/>
	<input type="hidden" id="atype" name="atype" value="<%=avvo.getAtype()%>"/>
</form>
	</div>
	</div>
