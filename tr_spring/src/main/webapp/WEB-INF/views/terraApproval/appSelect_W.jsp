...
<nav class="navbar navbar-expand-sm navbar-dark sticky-top" style="background-color: #C5E0B4;padding-right:0;">
					...
					<li class="mb-1">
					  ...
					  <div class="collapse show" id="home-collapse">
						<ul class="btn-toggle-nav list-unstyled fw-normal pb-1 small">
						  ...
						  <li><div id="nowSelected" class="idx" onclick="javascript:location.href='/tr_spring/appSelect/W.tr'"><img src="/tr_spring/resources/images/hourglass.png" class="icon_img"><span class="link-dark rounded" style="font-size:20px;margin-left:12px">대기</span></div></li>
	...
	<tr class="contents">	
		...
		<td><%= CodeUtil.nowStep(advo.getAlevel(), advo.getAapp(), uList.get(0).getMnum()) %></td>		
	</tr>	
	<%
			}
		}
	...
	<tbody>
	<%if(prList.size() > 0 && prList != null && pList.size() > 0 && pList != null){ %>
	<%
		for(int i=0; i < prList.size(); i++){
			
			AppDocVO advo = (AppDocVO)prList.get(i);
	%>					
	<tr class="contents">
		<td><%= advo.getAnum() %><input type="hidden" value=<%= advo.getAnum() %>></td>
		<td class="subj"><%= advo.getSubject() %></td>	
		...
		<td><%= advo.getInsertdate() %></td>			
		<td><%= CodeUtil.docType(advo.getAtype())%></td>		
		<td><%= CodeUtil.nowStep(advo.getAlevel(), advo.getAapp(), pList.get(0).getMnum()) %></td>		
	</tr>	
	<%
		}
	%>
