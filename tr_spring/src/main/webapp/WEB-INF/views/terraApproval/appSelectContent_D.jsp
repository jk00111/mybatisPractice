...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		if (<%=f%> == -1 || <%=Integer.parseInt(cList.get(0).getAlevel())%> == 9) {
...
<table>
	<tr>
		...
		<td colspan="2">&nbsp;<%=advo.getAnum() %></td>
	</tr>
	<tr>
		<td class="mpo">기안 부서</td>
		<td colspan="2">&nbsp;<%=CodeUtil.dappVal(advo.getMdeptno())%></td>
		<td class="mpo">기안자</td>
		<td colspan="2">&nbsp;<%=advo.getMname()%></td>
	</tr>
	<tr>
		<td class="mpo">보존 연한</td>
		<td colspan="2">&nbsp;<%=advo.getDeletedate()%> 년</td>
		<td class="mpo">보안 등급</td>
		<td colspan="2">&nbsp;<%=CodeUtil.aauthVal(advo.getAauth())%></td>
	</tr>
	<tr>
		<td class="mpo">기안 일시</td>
		<td colspan="2">&nbsp;<%=advo.getInsertdate()%></td>
		<td class="mpo">완료 일시</td>
		<td colspan="2">&nbsp;<%=advo.getFindate()%></td>
	</tr>
	<tr>
	...
	<br>
	<div style="width:780px"><h2><%=advo.getDsubject()%></h2>
	<hr>
	<p><%=advo.getDmemo()%></p></div>
	<input type="hidden" id="alevel" name="alevel"/>
	<input type="hidden" id="aconfirm" name="aconfirm"/>
	<input type="hidden" id="aapp" name="aapp" value="<%=advo.getAapp()%>"/>
	<input type="hidden" id="anum" name="anum" value="<%=advo.getAnum()%>"/>
	<input type="hidden" id="atype" name="atype" value="<%=advo.getAtype()%>"/>
</form>
	</div>
	</div>
